# Import necessary libraries 
from dash import html
import dash_bootstrap_components as dbc

# base64 image string
img = "data:image/png;base64,"

# Define the page layout
layout = dbc.Container([
    html.Br(),html.Br(),
    dbc.Row([
        
        html.Center(html.H4("Workload Heuristics Analysis Machine", style={"color": "#003c71", 'font-size': '1.2rem'})),
        html.Center(html.P("For more info, please refer to our Wiki page.", style={"color": "#003c71", 'font-size': '1rem'})),
        html.Br(),
        html.Br(),
        dbc.Col([
             dbc.Card(
                dbc.CardBody(
                    [
                        html.Center("Trace-Analytics", style={'font-size': '1.2rem', 'cursor' : 'pointer', 'text-decoration': 'underline'}), 
                        #html.Img(src={img}),                       
                        
                    ]
                ),
                style={"width": "18rem", 'font-size': '1.2rem', 'background-color' : '#0071c5', 'color' : 'whitesmoke'},
            )
        ]),

        dbc.Col([
             dbc.Card(
                dbc.CardBody(
                    [
                        html.Center("Workloads", style={'font-size': '1.2rem', 'cursor' : 'pointer', 'text-decoration': 'underline'}),
                        
                    ]
                ),
                style={"width": "18rem", 'font-size': '1.2rem', 'background-color' : '#0071c5', 'color' : 'whitesmoke'},
            )
        ]),

        dbc.Col([
             dbc.Card(
                dbc.CardBody(
                    [
                        html.Center("Tools", style={'font-size': '1.2rem', 'cursor' : 'pointer', 'text-decoration': 'underline', 'color' : 'whitesmoke'}),
                        
                    ]
                ),
                style={"width": "18rem", 'font-size': '1.2rem', 'background-color' : '#0071c5'},
            )
        ])
    ])
])
